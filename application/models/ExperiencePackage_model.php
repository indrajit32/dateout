...
class ExperiencePackage_model extends CI_Model
{
    ...
    public function getProducts($limit = null, $start = null, $big_get, $vendor_id = false, $key_name)
    {
        ...
        if(!empty($key_name) && isset($key_name)){
          if($key_name=='top_experience'){
            $this->db->where('products.display_top_experience', 'Yes');
          }
        }
        $this->db->select('vendors.url as vendor_url, products.expectation_folder, products.id, products.image, products.display_top_experience, products.discount_percent,
    ...
    {
        $this->db->where('products.display_top_experience', 'Yes');
        $this->db->select('products.id as experience_id, products.image, products.display_top_experience, products.discount_percent,
                  products.position, products_translations.title');
        $this->db->join('products_translations', 'products_translations.for_id = products.id', 'left');
        $this->db->join('vendors', 'vendors.id = products.vendor_id', 'left');
        $this->db->where('products_translations.abbr', MY_LANGUAGE_ABBR);
        $this->db->where('visibility', 1);
        ...
        if($query->num_rows()> 0){        
          foreach($query->result_array() as $val){
            ...
            $this->db->where('packages.experience_id', $val['experience_id']);
            $this->db->limit(1,0);
            $this->db->order_by('time_update', 'desc');
            $query1 = $this->db->get('packages');
            if ($query1->num_rows() > 0) {
              $val['is_available']= 'Yes';
